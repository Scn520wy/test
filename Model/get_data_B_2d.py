...
#         x_test = np.load('./data/test_B_bone.npy')
...
def extract_2d_pose(joint_data: np.ndarray) -> np.ndarray:
    # Assuming the input joint_data shape is (N, 3, 300, 17, 2)
    # We need to extract the 2D poses and retain the same shape
    return joint_data[:, :2, :, :, :]  # Here we can just return it as is since it's already in the desired shape
...
def process_data(train_data_path: str, test_data_path: str, train_label_path: str, test_label_path: str, save_path: str):
    ...
    # 原有的预处理步骤
    x_train = extract_2d_pose(x_train)
